...
class Buttons(object):
    ...
    def __init__(self):
        self.last_pressed_time = rospy.Time.now()
    ...
    def _fear_light_update(self, msg):
        self.fear_led_status = msg.type

    def _hope_light_update(self, msg):
        self.hope_led_status = msg.type
    ...
    def update_leds(self):
        if gpio_available:
            now = rospy.Time.now()
            # Hope LED update
            if self._hope_light_update == LightStatus.ON:
                GPIO.output(self.PIN_LED_HOPE, True)
                self.last_hope_led_update_time = rospy.Time.now()
            elif self._hope_light_update == LightStatus.OFF:
                GPIO.output(self.PIN_LED_HOPE, False)
                self.last_hope_led_update_time = rospy.Time.now()
            elif self._hope_light_update == LightStatus.FAST_BLINK:
                if now > self.last_hope_led_update_time + self.FAST_BLINK_DURATION:
                    self._hope_led_on = not self._hope_led_on
                    GPIO.output(self.PIN_LED_HOPE, self._hope_led_on)
                    self.last_hope_led_update_time = now
            elif self._hope_light_update == LightStatus.SLOW_BLINK:
                if now > self.last_hope_led_update_time + self.SLOW_BLINK_DURATION:
                    self._hope_led_on = not self._hope_led_on
                    GPIO.output(self.PIN_LED_HOPE, self._hope_led_on)
                    self.last_hope_led_update_time = now
            else:
                rospy.loginfo("No LED change")
            # Fear LED update
            if self._fear_light_update == LightStatus.ON:
                GPIO.output(self.PIN_LED_FEAR, True)
                self.last_fear_led_update_time = rospy.Time.now()
            elif self._fear_light_update == LightStatus.OFF:
                GPIO.output(self.PIN_LED_FEAR, False)
                self.last_fear_led_update_time = rospy.Time.now()
            elif self._fear_light_update == LightStatus.FAST_BLINK:
                if now > self.last_fear_led_update_time + self.FAST_BLINK_DURATION:
                    self._fear_led_on = not self._fear_led_on
                    GPIO.output(self.PIN_LED_FEAR, self._fear_led_on)
                    self.last_fear_led_update_time = now
            elif self._fear_light_update == LightStatus.SLOW_BLINK:
                if now > self.last_fear_led_update_time + self.SLOW_BLINK_DURATION:
                    self._fear_led_on = not self._fear_led_on
                    GPIO.output(self.PIN_LED_FEAR, self._fear_led_on)
                    self.last_fear_led_update_time = now
